# Get current git repository branch
function git_cur_branch()
{
     echo "$(git branch --contains HEAD 2> /dev/null | grep \* | sed "s/\* //;s/\(.*\)/(\1)$(git status --ahead-behind 2> /dev/null | sed -n 's/Your branch is ahead of .*\([0-9]\+\) commits\?\./ \1♔/p') /;")"
}
...
function runDocker() {
    img=$1
    cmd=$2
    docker run -it --rm $cmd $img /bin/bash
}

function dockerWithSSH() {
    proj=$1
    img=$2

    if [[ -z $img ]]; then
        img="registry.gitlab.cdn77.eu/ci-cd/images/go1.14"
    fi

    docker run -it --rm \
                -v $(pwd)/$proj/:/project/$proj \
...
function fmt_version {
    echo $(eval $(git describe --tags --abbrev=6 --always | \
		sed 's/^v//' | \
